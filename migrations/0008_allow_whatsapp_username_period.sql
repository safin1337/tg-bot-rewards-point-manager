...
ALTER TABLE customers RENAME TO customers_v206;
  ...
  whatsapp_username TEXT COLLATE NOCASE UNIQUE
    CHECK (
      ...
      OR (
        ...
        AND substr(whatsapp_username, 1, 1) != '.'
        AND substr(whatsapp_username, -1, 1) != '.'
        AND instr(whatsapp_username, '..') = 0
      )
    ),
...
CREATE TABLE _v207_customer_copy_guard (
  mismatch_count INTEGER NOT NULL CHECK (mismatch_count = 0)
);

INSERT INTO _v207_customer_copy_guard (mismatch_count)
SELECT COUNT(*)
FROM (
  SELECT
    id, whatsapp_number, phone_last4, phone_last5,
...
DROP TABLE _v207_customer_copy_guard;

-- Rebuild every direct customer-dependent table before removing the renamed
-- V2.0.6 customer table so all foreign keys target the canonical table.
ALTER TABLE transactions RENAME TO transactions_v206;
...
SELECT
  ...
  rounded_reward_after_bdt, transaction_reward_rounded_bdt, note,
  telegram_update_id, created_at_utc
FROM transactions_v206;
DROP TABLE transactions_v206;
...
ALTER TABLE mutation_receipts RENAME TO mutation_receipts_v206;
...
SELECT
  ...
  balance_before_units, balance_after_units, rounded_reward_before_bdt,
  rounded_reward_after_bdt, transaction_reward_rounded_bdt, completed_at_utc
FROM mutation_receipts_v206;
DROP TABLE mutation_receipts_v206;
...
ALTER TABLE leaderboard_aggregates RENAME TO leaderboard_aggregates_v206;
...
SELECT
  period_type, period_key, generation, customer_id, earned_point_units,
  first_qualifying_earning_at_utc, updated_at_utc
FROM leaderboard_aggregates_v206;
DROP TABLE leaderboard_aggregates_v206;
...
ALTER TABLE conversation_states RENAME TO conversation_states_v206;
...
SELECT
  ...
  selected_customer_id, search_query, search_page, payload_json,
  created_at_utc, updated_at_utc, expires_at_utc, operation_started_update_id
FROM conversation_states_v206;

DROP TABLE conversation_states_v206;
CREATE INDEX idx_conversation_states_expiry ON conversation_states(expires_at_utc);

DROP TABLE customers_v206;
...
CREATE TABLE _v207_foreign_key_guard (
  violation_count INTEGER NOT NULL CHECK (violation_count = 0)
);

INSERT INTO _v207_foreign_key_guard (violation_count)
SELECT COUNT(*) FROM pragma_foreign_key_check;

DROP TABLE _v207_foreign_key_guard;
